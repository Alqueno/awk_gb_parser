...
BEGIN { 
	...
	ACC="Accession"; Def="Definition"; src_pos[0]="nt_pos"; hst[0]="Host"; PMID="Pubmed_ID"; is[0]="Isolate"; cl[0]="Clone"; co[0]="Country"; org[0]="Organism"; sty[0]="Serotype"; cd[0]="Collection_date"; sr[0]="Strain"; op=0; src=0 };
/DEFINITION/ { 
	#write down the previous record, iterate between all the sources found
	for (f in src_pos) {
	printf ACC "\t" Def "\t" PMID "\t" src_pos[f] "\t" is[f]  "\t" hst[f] "\t" cl[f] "\t" co[f] "\t" cd[f] "\t" org[f] "\t" sty[f] "\t" sr[f] "\n";
    };
	#clear stored info from previous recors and starts to read the posible multiline definition field.
	$1=""; gsub(/ +$/,""); Def=gensub(/%/, "pct", "g") ; delete org; delete sty; delete is; delete cl; delete co; delete cd; delete sr; ACC=""; PMID=""; delete src_pos; src=0; op=1 };
/^            / { 
